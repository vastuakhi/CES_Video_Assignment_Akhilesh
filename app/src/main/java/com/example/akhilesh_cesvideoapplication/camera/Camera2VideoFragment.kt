package com.example.akhilesh_cesvideoapplication.camera
...
import com.example.akhilesh_cesvideoapplication.AppDataBase
...
class Camera2VideoFragment : Fragment(), View.OnClickListener,
    ...
    override fun onClick(view: View) {
        when (view.id) {
            ...
            R.id.info -> {
                if (activity != null) {
                    fragmentManager?.beginTransaction()?.add(R.id.container, VideoFileList())
                        ?.addToBackStack("VideoFileList")
                        ?.commit()
                }
            }
        }
    ...
    private fun stopRecordingVideo() {
        ...
        var videoInfo = VideoFileModel(
            videoFileId = 0,
            videoPath = nextVideoAbsolutePath,
            videoName = filename,
            isFileUploaded = 0
        )
        databaseHandler.addVideoInfo(videoInfo)
